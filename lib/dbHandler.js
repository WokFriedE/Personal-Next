...
export function genericGET(table) {
    ...
    return async () => {
        try {
            const data = await db.all(`SELECT * FROM ${table} WHERE is_active = 1`);
            return data;
        } catch (err) {
            console.error(err);
            return [];
...
// ========================
export async function extracurricularGET() {
...
// Delete request
export async function extracurricularDELETE(title = "", method = "all") {
    try {
        if (task.method === "all") {
...
// Delete request
export async function projectDELETE(title = "", method = "all") {
    try {
        if (method === "all") {
