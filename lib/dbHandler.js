...
export function genericDELETE(table) {
    ...
    return async (id = -1, method = "all", disable = true) => {
        try {
            if (!disable && method !== "toggleAll") {
                if (method === "all") await db.run(`DELETE FROM ${table}`);
                else if (id) await db.run(`DELETE FROM ${table} WHERE id = ?`, id);
                else return false;
                return true;
            } else {
                ...
                else if (id) await db.run(`UPDATE ${table} SET is_active = (CASE WHEN is_active = 0 THEN 1 ELSE 0 END) WHERE id = ?`, id);
                else return false;
                return true;
            }
        } catch (err) {
...
export async function projectGET(activeOnly = true) {
    const projectData = await db.all(`SELECT * FROM projects ${activeOnly ? "WHERE is_active = 1" : ""} ORDER BY current DESC, end DESC, start DESC`);
        ...
        temp.forEach(async (taskI) => {
            ...
            const currId = await db.run(
                ...
                RETURNING id `,
                {
                    ...
                    $imgSrc: taskI.imgSrc ?? "/images/no-img.png",
            ...
            console.log(currId.lastID);
            if (taskI.tech) {
                taskI.tech.forEach(async (techItem) => {
                    if (techItem.language_id) {
                        db.run(
                            `INSERT INTO projectLangs (language_id, project_id) VALUES ($lid, $pid)
                            ON conflict do UPDATE set is_active=$is_active`,
                            {
                                $lid: techItem.language_id,
                                $pid: currId.lastID,
                                $is_active: !techItem.is_active,
                            }
                        );
                    } else if (techItem.tool_id) {
                        db.run(
                            `INSERT INTO projectTools (tool_id, project_id) VALUES ($tid, $pid)
                            ON conflict do UPDATE set is_active=$is_active`,
                            { $tid: techItem.tool_id, $pid: currId.lastID, $is_active: !techItem.is_active }
                        );
                    }
                });
            }
...
// Update request
export async function projectUPDATE(taskI, currId) {
    const temp = [];
    if (Array.isArray(taskI)) {
        console.error("Project Update fail");
        return false;
    }
    ...
    try {
        ...
        await db.run(
            `UPDATE projects set current=$current, start=$start, end=$end, description=$description, imgSrc=$imgSrc, link=$link, github=$github, videoSrc=$videoSrc, is_active=$is_active WHERE id=$id`,
            {
                $id: currId,
                $current: taskI.current ?? false,
                $start: taskI.start ?? null,
        ...
        db.run(`UPDATE projectLangs SET is_active=0 WHERE project_id=$pid`, { $pid: currId });
        db.run(`UPDATE projectTools SET is_active=0 WHERE project_id=$pid`, { $pid: currId });
        if (taskI.tech) {
            taskI.tech.forEach(async (techItem) => {
                if (techItem.language_id) {
                    db.run(
                    ...
                    db.run(
                        `INSERT INTO projectTools (tool_id, project_id) VALUES ($tid, $pid)
                            ON conflict do UPDATE set is_active=$is_active`,
                        { $tid: techItem.tool_id, $pid: currId, $is_active: !techItem.is_active }
                    );
                }
            });
        }
...
export async function projectDELETE(title = "", method = "all", disable = true) {
    try {
        if (!disable && method !== "toggleAll") {
            ...
            } else if (title) await db.run(`DELETE FROM projects WHERE title = ?`, title);
            else return false;
            return true;
        } else {
            if (method === "all") {
