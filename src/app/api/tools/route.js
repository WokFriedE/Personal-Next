...
export async function GET(req, res) {
    ...
    const langData = await db.all("SELECT * FROM tools WHERE is_active = 1");
...
export async function POST(req, res) {
    ...
    temp.forEach(async (task) => {
        ...
        await db.run(
            `INSERT INTO tools (name, proficiency, type, description, icon, is_active) VALUES ($name, $proficiency, $type, $description, $icon, $is_active)
